...
public class MetaData_v2_1 {
    ...
    public static Serie createSerie(Group group) {
        ...
        serie.setInh1(TextUtil.truncate(group.getMainDescription(), 4096));
        return serie;
    }
    ...
    protected static Omroepen createOmroepen(MediaObject mediaObject, BroadcasterService broadcasterService) {
        ...
        for (Broadcaster broadcaster : broadcasters) {
            if (broadcasterService != null) {
                Broadcaster b = broadcasterService.find(broadcaster.getId());
                if (b == null) {
                    log.warn("Could not find broadcaster {} in {}", broadcaster.getId(), broadcasterService);
                } else {
                    broadcaster = b;
                }
            }
    ...
    protected static Personen createPersonen(MediaObject mediaObject) {
        ...
        for (Person person : persons) {
            ...
            persoon.setVoornaam(person.getGivenName());
            persoon.setAchternaam(person.getFamilyName());
            Rol rol = new Rol();
            rol.setValue(person.getRole().toString());
            rol.setId(person.getRole().ordinal());
            persoon.setRol(rol);
