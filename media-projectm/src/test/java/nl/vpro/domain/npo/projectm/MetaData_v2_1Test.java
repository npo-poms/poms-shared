...
import nl.vpro.domain.npo.projectm.metadata.v2_1.Aflevering;
...
public class MetaData_v2_1Test {
    ...
    public void testCreateAflevering() {
        ...
        assertThat(aflevering.getPersonen().getPersoon().get(0).getVoornaam()).isEqualTo("Pietje");
        assertThat(aflevering.getPersonen().getPersoon().get(0).getAchternaam()).isEqualTo("Puk");
        assertThat(aflevering.getPersonen().getPersoon().get(0).getRol().getId()).isEqualTo(11);
        assertThat(aflevering.getPersonen().getPersoon().get(0).getRol().getValue()).isEqualTo("Componist");

        assertThat(aflevering.getPersonen().getPersoon().get(1).getVoornaam()).isEqualTo("Jan");
        assertThat(aflevering.getPersonen().getPersoon().get(1).getAchternaam()).isEqualTo("Jansen");
        assertThat(aflevering.getPersonen().getPersoon().get(1).getRol().getId()).isEqualTo(0);
        assertThat(aflevering.getPersonen().getPersoon().get(1).getRol().getValue()).isEqualTo("Regisseur");
    ...
    protected Program getTestProgram(MediaProvider provider) {
        ...
        return MediaBuilder.program()
            ...
            .lastModified(LocalDateTime.of(2011, 3, 23, 14, 32, 5).atZone(Schedule.ZONE_ID))
