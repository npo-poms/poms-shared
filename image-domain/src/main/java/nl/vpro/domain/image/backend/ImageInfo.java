 ...
 *  ii.getMimeType() + &quot;, &quot; + ii.getWidth() + &quot; x &quot; +
 *  ii.getHeight()
 *           + &quot; pixels, &quot; + ii.getBitsPerPixel() + &quot; bits
 *  per pixel, &quot; + ii.getNumberOfImages() + &quot; image(s), &quot;
 ...
 * {@link #setDetermineImageNumber} with <code>true</code> as argument to identify animated GIFs ({@link
 * #getNumberOfImages()} will return a value larger than <code>1</code>).</li>
 ...
 * two methods to return the physical width and height of the image in dpi: {@link #getPhysicalWidthDpi()} and {@link
 * #getPhysicalHeightDpi()}. If the physical resolution could not be retrieved, these methods return <code>-1</code>.
 * </li>
 ...
 * which tries "read" whenever "skip(Bytes)" returns a result of 0. The old method didn't work with certain input stream
 * types on truncated data streams. Thanks to Martin Leidig for reporting this and sending in code. Released as 1.8.</li>
 * </li>
...
public class ImageInfo {
    ...
    private static final Map<String, Integer> extensionToFormat;
    ...
    static {
        ...
        aMap.put("ppm", 9);
        aMap.put("psd", 10);
        extensionToFormat = Collections.unmodifiableMap(aMap);
    }
    ...
    public boolean check() {
        ...
        try {
            ...
            log.warn("error occured during conversion", ioe);
            return false;
        }
    }
     ...
     * @throws IllegalArgumentException if index is smaller than 0 or larger than or equal to the number of comments
     *                                  retrieved
    ...
    /**
     * Get the format corresonding to an extension
     *
     * @return the format number
     */
    public static int extensionToFormat(String extension) {
        if(extension != null) {
    ...
    public static String extensionToMimeType(String extension) {
        if(extension != null) {
            Integer format = extensionToFormat.get(extension.toLowerCase());
            if(format != null && format >= 0 && format < MIME_TYPE_STRINGS.length) {
                return MIME_TYPE_STRINGS[format];
            } else {
                return null;
            }
    ...
    /**
     * If {@link #check()} was successful and {@link #setCollectComments(boolean)} was called with <code>true</code>
     * as argument, returns the number of comments retrieved from the input image stream / file. Any number &gt;= 0 and
    ...
    private static void printCompact(String sourceName, ImageInfo imageInfo) {
        final String SEP = "\t";
        log.debug(sourceName + SEP + imageInfo.getFormatName() + SEP + imageInfo.getMimeType() + SEP
     ...
     * <code>true</code> as argument, the actual number of images can be queried via {@link #getNumberOfImages()}
